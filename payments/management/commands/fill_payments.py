...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Очистка старых платежи ({payment_count} шт.)")
        )
        ...
        for course in courses:
            ...
            Payment.objects.create(
                user=user, course=course, lesson=None, amount=4990, method="transfer"
            )
            created += 1
        ...
        for lesson in lessons:
            user = random.choice(users)
            Payment.objects.create(
                user=user, course=None, lesson=lesson, amount=990, method="cash"
            )
            created += 1
